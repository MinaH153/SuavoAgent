...
**Learning-hardening wave (branch `fix/qa-learning-hardening`):**
- ✅ **Dead auto-rule-approval gate — FIXED.** New pure, tested `AutoRuleApprovalGate.AdmitApproved` wired into the RuleEngine DI factory: only operator-Approved auto-rules load; a rule with no `auto_rule_approvals` row (hand-authored/embedded override) loads normally; Pending/Shadow/Rejected blocked. Chose filter-at-load over staging (the DB status is the single source of truth — no file/status drift). Shadow blocked deliberately (engine shadow MODE is per-evaluation, not per-rule; no on-device `shadow_runs` writer).
- ⏸ **Replay terminal-step TOCTOU — deferred to harvest-resumption** (documented at the site). `PostconditionEvaluator` is v1 change-detection; middle-step wrong-landings are caught by the next iteration's state-match, but the terminal step has no next step. The proper fix banks a `FinalStateHash` and asserts it on the terminal step — which needs the (hard-stopped Phase-3B) harvester to CAPTURE the final post-state + a skill-schema change, exercisable only with the harvest→bank→replay round-trip live. Worst case today is navigation-only (auto-replay is click-family, type-steps OFF).

**Still deferred (each deserves its own task):** Setup **I-4** (Helper crash-relaunch backoff — Broker supervision loop), **PhiTextScrubber ReDoS sentinel** test (the 1s rule timeout is baked into the statically-compiled rules — needs a production seam to trigger deterministically), and **SQLCipher migration on a Windows CI runner** (needs Skip→WindowsFact + a Core.Tests-on-Windows job — can't verify from macOS).
