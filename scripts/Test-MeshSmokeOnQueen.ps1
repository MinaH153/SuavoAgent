...
function Invoke-WithTimeout {
    ...
    .DESCRIPTION
        ...
        smoke runner indefinitely (a recurring class of failure on Queen — see
        memory rule `feedback-agent-watchdog-stalls-2026-05-04`). On timeout,
        kills the process tree and merges captured stderr into the stdout log
        so existing tail/grep logic remains untouched.
    #>
    param(
        [Parameter(Mandatory)][string]$FilePath,
        [Parameter(Mandatory)][string[]]$ArgumentList,
        [Parameter(Mandatory)][string]$LogPath,
        [int]$TimeoutSeconds = 900,  # 15 min default
