# SuavoAgent Quick Install - just downloads and installs, no PMS discovery
...
foreach ($s in @("SuavoAgent.Broker","SuavoAgent.Core")) {
    ...
    if ($svc) { Stop-Service $s -Force -ErrorAction SilentlyContinue; Start-Sleep 1; sc.exe delete $s | Out-Null; Start-Sleep 1 }
}
...
Start-Service SuavoAgent.Core; Start-Sleep 3; Start-Service SuavoAgent.Broker -ErrorAction SilentlyContinue
