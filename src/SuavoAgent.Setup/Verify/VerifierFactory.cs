...
/// console installer verify identically (Services → Pipe → Brain → Cloud auth → Workstation). A Fail on any gate
...
public static class VerifierFactory
{
    public static PostInstallVerifier BuildDefault() => new(new Func<CancellationToken, Task<GateResult>>[]
    {
        ...
        _ => Task.FromResult(new WorkstationReadinessProbe().Check()),
    });
}
