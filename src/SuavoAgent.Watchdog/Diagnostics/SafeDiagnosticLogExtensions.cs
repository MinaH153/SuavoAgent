...
/// <summary>PHI-safe caught-exception logging boundary for Watchdog.</summary>
internal static class SafeDiagnosticLogExtensions
{
    ...
    public static void LogSafeCritical(
        ...
        Exception exception,
        [CallerMemberName] string operation = "unknown") =>
        Write(logger, LogLevel.Critical, exception, operation);
    ...
    private static void Write(ILogger logger, LogLevel level, Exception exception, string operation)
    {
        logger.Log(
            ...
            $"watchdog.{NormalizeIdentifier(operation)}.exception",
